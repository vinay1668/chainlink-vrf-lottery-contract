...
contract lo is VRFConsumerBase{
   ...
   function cliamPrize(uint _round) public{
       ...
       require(available , "The randomNumber is not generated yet");
   ...
   event WinningTicket(uint number);
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        randomNumber = randomness.mod(2).add(1);
        available = true;
        emit WinningTicket(randomNumber);
    }
